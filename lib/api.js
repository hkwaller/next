...
exports.getStationList = function(lat, lng, numberofstops, callback) {
  ...
  request.get(url, function(res) {
    ...
    stations.forEach(function(station, index) {
      ...
      station.Preferance = (preferredStationsByGeoHash[geoHash] && preferredStationsByGeoHash[geoHash][station.ID]) || 0;
      station.Index = index;
    });

    stations.sort(function (a, b) {
      var dPreferance = b.Preferance - a.Preferance;
      var dIndex = a.Index - b.Index;
      return dPreferance === 0 ? dIndex : dPreferance;
    });
...
exports.getPreferredStation = function (lat, lng) {
  ...
  var largestPreferance = 0;
  var preferredStation = null;
  if (preferredStations) {
    Object.keys(preferredStations).forEach(function (ID) {
      if (preferredStations[ID] > largestPreferance) {
        largestPreferance = preferredStations[ID];
        preferredStation = ID;
      }
    });
  }
